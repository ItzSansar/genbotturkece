...
client.once('ready', () => {
    ...
    client.user.setActivity(`.gg/hediyeowo ❤️ gg./freeaccgen`, { type: "WATCHING" }); // Set the bot's activity status
...
client.on('message', (message) => {
	...
	if (config.command.notfound_message === true && !client.commands.has(command)) {
        return message.channel.send(
            ...
            .setDescription(`Üzgünüm, ama bulamıyorum \`${command}\` Komut!`)
	...
	try {
		...
		if (config.command.error_message === true) {
            message.channel.send(
                ...
                .setDescription(`yürütülürken bir hata oluştu \`${command}\` komut!`)
                .addField('Hata404', `\`\`\`js\n${error}\n\`\`\``)
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp()
            );
