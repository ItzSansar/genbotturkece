	...
	execute(message, args) {
        ...
        if (message.channel.id === config.genChannel) {
                    ...
                    if (!error) {
                        ...
                        if (position === -1) {
                            return message.channel.send(
                                ...
                                .setDescription(`bulamıyorum \`${args[0]}\` stoklarımda hizmet!`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
                        ...
                        if (position !== -1) {
                            ...
                            fs.writeFile(filePath, data, function (error) {
                                message.channel.send(
                                    ...
                                    .setDescription(`Dm Kutunuzu kontrol edin ${message.author}! *Mesajı almadıysanız, lütfen Dm Ayarlarınızı Kontrol Ediniz*`)
                                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                    .setTimestamp()
                                );
            ...
            message.channel.send(
                ...
                .setDescription(`Bu kanalda \`gen\` komutunu kullanamazsınız! <#${config.genChannel}>!`)
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                .setTimestamp()
            );
        };
